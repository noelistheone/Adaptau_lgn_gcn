...
#     """
#     def __init__(self, n_hops, n_users, interact_mat,
#                  edge_dropout_rate=0.5, mess_dropout_rate=0.1, eps=0.5):
#         super(GraphConv, self).__init__()
#         self.eps = eps
#         self.interact_mat = interact_mat
#         self.n_users = n_users
...
#     def sparse_dropout(self, x, rate=0.2):
#         noise_shape = x._nnz()
...
#     # def contrastive_loss(self, anchor, positive, temperature=0.5):
...
#     def calculate_cf_loss(self, user_online, user_target, item_online, item_target, alpha = 0.5):
#         user_online, item_online = self.predictor(user_online), self.predictor(item_online)
#         criterion = nn.MSELoss()
#         user_online = user_online.view(-1, user_online.size(0))
#         item_online = item_online.view(-1, item_online.size(0))
#         user_online, item_online = torch.transpose(user_online, 0, 1), torch.transpose(item_online, 0, 1)
#         # Reconstruction (Denoising) Loss
#         reconstruction_loss_user = criterion(user_online, user_target)
#         reconstruction_loss_item = criterion(item_online, item_target)
#         reconstruction_loss = reconstruction_loss_user + reconstruction_loss_item

#         # Contrastive Loss
#         # contrastive_loss_user = self.contrastive_loss(user_online, user_target, self.temperature)
#         # contrastive_loss_item = self.contrastive_loss(item_online, item_target, self.temperature)
#         # total_contrastive_loss = contrastive_loss_user + contrastive_loss_item
