...
def read_cf_amazon_kindle(file_name):
    inter_mat = np.loadtxt(file_name, dtype=np.int32)
    mat = inter_mat[:, :-1]
    mat[:, [0, 1]] = mat[:, [1, 0]]
    return mat

def read_cf_amazon(file_name):
    inter_mat = np.loadtxt(file_name, dtype=np.int32)  # [u_id, i_id, rate]
    inter_mat = inter_mat[:, :-1]
    #sorted_indices = np.argsort(inter_mat[:, 0])
    #sorted_matrix = inter_mat[sorted_indices]
    #print(sorted_matrix)
    inter_mat = inter_mat - 1
    return inter_mat

def read_cf_amazon_1(file_name):
    inter_mat = np.loadtxt(file_name, dtype=np.int32)  # [u_id, i_id, rate]
    inter_mat = inter_mat[:, :-1]
    #sorted_indices = np.argsort(inter_mat[:, 0])
    #sorted_matrix = inter_mat[sorted_indices]
...
def read_cf_Epinions(file_name):
...
def read_cf_yelp2018(file_name):
    ...
    for l in lines:
        tmps = l.strip()
        inters = [int(i) for i in re.split(r'[ :]', tmps) if i]
        u_id, pos_ids = inters[0], inters[1:]
        pos_ids = list(set(pos_ids))
        for i_id in pos_ids:
            inter_mat.append([u_id, i_id])
    #print(np.array(inter_mat))
    return np.array(inter_mat)

def read_cf_yelp2018_1(file_name):
    inter_mat = list()
    lines = open(file_name, "r").readlines()
    for l in lines:
        tmps = l.strip()
...
def load_data(model_args, logger):
    ...
    elif dataset == "amazon-kindle":
        read_cf = read_cf_amazon_kindle
    elif dataset == "iFashion":
        read_cf = read_cf_amazon_1
    elif dataset == "amazon-beauty":
        read_cf = read_cf_yelp2018_1
    elif dataset == "Epinions" or dataset == "last-fm" or dataset == "hetrec2011-delicious-2k" or dataset == "tmall":
        read_cf = read_cf_Epinions
    else:
    ...
    if dataset == 'yelp2018' or dataset == "amazon-book" or dataset == "gowalla" or dataset == 'ml-1M' or dataset == "citeulike-new" or dataset == "douban-book" or dataset == "amazon-beauty" or dataset == "amazon-kindle" or dataset == "iFashion" or dataset == "Epinions" or dataset == "last-fm" or dataset == "hetrec2011-delicious-2k" or dataset == "tmall":
        valid_cf = test_cf
    else:
